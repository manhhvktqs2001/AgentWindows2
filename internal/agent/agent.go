...
func (a *Agent) Start() error {
	...
	if err != nil {
		// Do not stop the agent if server is temporarily unreachable.
		// Run in offline mode: monitors still generate local events; sending may fail but won't exit.
		a.logger.Warn("Server registration failed; running in offline mode: %v", err)
	}
